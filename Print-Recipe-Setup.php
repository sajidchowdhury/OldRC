...
if($_GET['product_id'] == 'New'){
    ...
    $print_material_id = $product_info['id'];
}
...
$DATA  = SETUP::SETUP_PRINT_RECIPE($_GET['related_id']);
$related_id = $DATA['id'];
...
<ul class="breadcrumb">
    ...
    <input type="hidden" name="previous_recipe" id="previous_recipe" value="">
                                ...
                                <div class="panel-body">      <b id="mess_box"></b>                                                                  
                                    ...
                                    <div class="row">
                                        ...
                                        <fieldset>
                                        <legend>Print Item Information</legend>

                                        <div class="form-group">
                                                <label class="col-md-3 control-label"> Material Name</label>
                                                <div class="col-md-9">                                                                                            
                                                <select id="print_material_id" name="print_material_id"  onchange="findRecipeSetupOrnot('receip_print','print_material_id',this.value)"  class="form-control select" data-live-search="true">
        <option value="">Select One</option>
        <?php 
    
    foreach ( $xml_raw_material_list->ROW as  $value ) { ?>
        <option <?php if($print_material_id == $value['id'] ){ ?> selected="selected" <?php }else{} ?>  value="<?php print $value['id'];?>"><?php print "$value[product_category] ::: $value[product_name]";?></option>
    <?php }
    ?>
...
<span   class="help-block text-danger" id="mess_load"></span>
 ...
 <?php } ?>
                                            ...
                                            <fieldset>
                                        <legend>RAW Item Information</legend>
 ...
 <?php } ?>
                                ...
                                <div class="panel-footer">
                                    <?php if($_GET['related_id'] == 'New' && $_GET['product_id'] == 'New'){ ?>
                                        <input type="button" id="add_cart_print_recipe" name="add_cart_print_recipe" class="btn btn-info pull-right" value="Add++"  >
                                        <?php  }else{?>
                                            <input style="margin-left:15px;" type="button" class="btn btn-warning pull-right" value="Update" id="add_cart_print_recipe" name="add_cart_print_recipe" >
                                            <a class="btn btn-info pull-right" href="Recipe/Print-Recipe-Setup/New/New" >Add New</a>
                                            <?php  }?>
                               
                                <input type="button" class="btn btn-danger pull-left" value="Final Data" onclick="" id="final_print_recipe" name="final_print_recipe" >
                                </div>
...
<?php include('print-recipe-cart.php');?>
</div>