...
if($_GET['related_id'] == 'New'){
    ...
    $ac_head_name = $DATA['transection_head_name'];
...
$pending_data = FIND::PENDING_TRANSACTION_POST('INCOME','ADD INCOME');
...
<div class="row">
          ...
          <input type="hidden" id="data_inserted_from" name="data_inserted_from" value="ADD INCOME">
...
<tr>
        <th>Ledger Head</th>
        <td colspan="4">
        <select name="ledger_id" id="ledger_id" class="form-control select" onchange="LedgerWiseData('INCOME',this.value)"  data-live-search="true">
...
foreach ($fetch_list as $fetch) { ?>
   <option  <?php if($ledger_id == $fetch['id'] ){ ?> selected="selected" <?php } else{ } ?> value="<?php print $fetch['id'];?>"> <?php print $fetch['name'];?> </option>';
...
<?php 
$qry = $conn_me->prepare("SELECT * FROM `setup_ac_head` where ( `account_type`  = 'INCOME' OR  `account_type`  = 'BOTH' )  ");
$qry->execute();
$fetch_list = $qry->fetchAll(PDO::FETCH_ASSOC);
foreach ($fetch_list as $fetch) { ?>
...
?>
  ...
  <tr <?php if ($_SESSION['USER_TYPE'] == 'Admin' ){ ?> <?php }else{ ?> style="display: none;" <?php } ?> >
    ...
    <th colspan="5">
    <?php if($related_id == 'new_id'){ }else{ ?>
        <a href="Account/INCOME/New" class="pull-right btn btn-success">Add New<a>

    <?php }?>
    <input type="button"   onclick="pending_income_transection()" class="btn btn-warning pull-right" value="Receive Now"  style="margin-right: 11px;">
    </th>
    </tr>
...
<?php if($pending_data > 0 ){ ?>
    <input type="button" id="pending_data_cal_id" value="<?php print $pending_data;?>" class="btn btn-danger block" onclick="get_pending_post('INCOME','ADD INCOME','INCOME');">
...
<div class="col-sm-4">
<input type="button" onclick="transectionSummery('INCOME','INCOME');" class="btn btn-info block" value="SEARCH DATA" id="search_data" name="search_data" ></td>
</div>
<div class="col-sm-4"></div>
