...
<?php 
                    ...
                    $total_carton = 0;
                    $total_vat = 0;
        $query = $conn_me->prepare("SELECT *  FROM `quotation_invoice_item`  where `poster` = '".$_SESSION['NEWERP_SESS_MEMBER_ID']."' AND `brunch_id` = '".$_SESSION['USER_BRUNCH']."' AND `status` = 'Pending' ORDER BY `id` DESC");
        $query->execute();
...
if($count > 0 ){
    ...
    <tr style="line-height: 0.5">
            ...
            <button class=""><a onClick="edit_sale_cart(
                ...
                <button class="" onClick="delete_cart_row('cart_quatation_entry','quotation_invoice_item','<?php print $fetch['id'];?>','No');"><span class="fa fa-times"></span></button>
            </td>
