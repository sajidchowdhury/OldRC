...
if($_GET['related_id'] == 'New'){
    ...
    $amount = ($DATA['transection_type'] == 'INCOME' ) ? $DATA['in_amount'] : $DATA['out_amount'];
...
$pending_data = FIND::PENDING_TRANSACTION_POST('BOTH','SUPPLIER-PAYMENT');
...
<div class="row">
          ...
          <input type="hidden" id="data_inserted_from" name="data_inserted_from" value="SUPPLIER-PAYMENT">
          ...
          <input type="hidden" id="ledger_id" name="ledger_id" value="3">
...
<td colspan="3">
<select id="transection_to_id" name="transection_to_id" required onchange="findTransectionDue(this.value,'Supplier','Brunch_Wise');"  class="form-control select" data-live-search="true">
...
<option  <?php  if($transection_to_id == $fetch['id']){?> selected="selected" <?php }else{ } ?>  value="<?php print $fetch['id'];?>"><?php print $fetch['mobile'] . '-' . $fetch['supplier_name'];?>
</option>
<?php } ?>
...
<?php if($pending_data > 0 ){ ?>
    <input type="button" id="pending_data_cal_id" value="<?php print $pending_data;?>" class="btn btn-danger block" onclick="get_pending_post('BOTH','SUPPLIER-PAYMENT','SUPPLIER-PAYMENT');">
...
<div class="col-sm-4">
<input type="button" onclick="transectionSummery('3','SUPPLIER-PAYMENT');" class="btn btn-info block" value="SEARCH DATA" id="search_data" name="search_data" ></td>
</div>
<div class="col-sm-4"></div>
