...
$permission_management = PERMISSION::pageCreatePermission($_SESSION['USER_TYPE'],$button_text,'create_spray_recipe_wise_demand','');
...
<tr>
    <td class="form-group required control-label">Spray From</td>
    <td  colspan="2">
...
<td  colspan="2">
<select id="spray_material_id" name="spray_material_id" onchange="spray_batch_calculation();"  class="form-control select" data-live-search="true">
    <option value="">Select One</option>
<?php 
        $qry = $conn_me->prepare("SELECT * FROM `receip_spray`  GROUP BY `spray_material_id` ");
        $qry->execute();
        $fetch_list = $qry->fetchAll(PDO::FETCH_ASSOC);
        foreach ($fetch_list as $fetch) {  
            $product_info = SETUP::SETUP_RAW_MATERIAL($fetch['spray_material_id']);

            ?>

        <option <?php if($spray_material_id == $fetch['spray_material_id']) { ?>  selected="selected" <?php }else{};?> value="<?php print $fetch['spray_material_id'];?>"><?php print $product_info['category'] . '-' .$product_info['product_name'];?>
        </option>
 <?php } ?>
...
<tr>
    ...
    <td  colspan="2"> <input type="number" class="form-control" value="<?php print $batch_quantity;?>"   onkeyup="spray_batch_calculation();" id="batch_quantity" name="batch_quantity" >
</tr>
...
<tr>
    <td  colspan="6" ><?php print  $permission_management['save_update_buton'];?></td>
                        ...
                        <?php include('cart_request_raw_material_spray.php');?> 
                        </div>
                        </div>
