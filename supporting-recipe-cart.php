...
if($_GET['related_id'] == 'New'){
    ...
    $query = $conn_me->prepare("SELECT *  FROM `receip_supporting_goods`  where `supporting_id` = '".$_GET['supporting_id']."' ORDER BY `id` DESC");
...
if($count > 0 ){
    ...
    <tr style="line-height: 0.5">
            ...
            <td>
            <button class="btn btn-default btn-rounded btn-sm"><a href="Recipe/Supporting-Recipe-Setup/<?php print $fetch['id'];?>/<?php print $fetch['supporting_id'];?>"><span class="fa fa-pencil"></span></a></button>
                <button class="btn btn-danger btn-rounded btn-sm" onClick="delete_cart_row('supporting-recipe-cart','receip_supporting_goods','<?php print $fetch['id'];?>','No');"><span class="fa fa-times"></span></button>
            </td>
