...
$permission_management2 = PERMISSION::pageCreatePermission($_SESSION['USER_TYPE'],'Final','final_demand','onclick="final_pending_print_demand();"');
...
$data = '';
$aa1 = '1';
$query = $conn_me->prepare("SELECT *  FROM `raw_print`  where `poster` = '".$_SESSION['NEWERP_SESS_MEMBER_ID']."' AND `status` = 'Pending' ORDER BY `date`,`time` DESC");
$query->execute();
...
foreach ($fetch_list as $fetch) {
    ...
    ?>
            <tr>
                ...
                <button  onClick="delete_print_recipe_wise_demand_and_item('<?php print $fetch['id'];?>');"><span class="fa fa-times"></span></button>
                </td>
            </tr>
...
$aa2 = '1';
$total_materials = 0;
$query = $conn_me->prepare("SELECT SUM(`demand_quantity`) AS `total_demand` ,`material_id` FROM `raw_print_item`  where `poster` = '".$_SESSION['NEWERP_SESS_MEMBER_ID']."' AND `status` = 'Pending' GROUP BY `material_id` ORDER BY `date`,`time` DESC");
$query->execute();
$fetch_list = $query->fetchAll(PDO::FETCH_ASSOC);
foreach ($fetch_list as $fetch) {
